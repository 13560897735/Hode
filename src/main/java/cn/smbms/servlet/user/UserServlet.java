...
//		}
	...
	private void updatePwd(HttpServletRequest request, HttpServletResponse response)
		...
		if(o != null && !StringUtils.isNullOrEmpty(newpassword)){
			...
			flag = context.getBean("userServiceImpl", UserService.class).updatePwd(((User)o).getId(),newpassword);
			if(flag){
	...
	private void modify(HttpServletRequest request, HttpServletResponse response)
		...
		if(context.getBean("userServiceImpl", UserService.class).modify(user)){
	...
	private void getUserById(HttpServletRequest request, HttpServletResponse response,String url)
		...
		if(!StringUtils.isNullOrEmpty(id)){
			//调用后台方法得到user对象
			User user = context.getBean("userServiceImpl", UserService.class).getUserById(id);
	...
	private void delUser(HttpServletRequest request, HttpServletResponse response)
		...
		if(delId <= 0){
			...
			if( context.getBean("userServiceImpl", UserService.class).deleteUserById(delId)){
	...
	private void userCodeExist(HttpServletRequest request, HttpServletResponse response)
		...
		if(StringUtils.isNullOrEmpty(userCode)){
			...
			User user =  context.getBean("userServiceImpl", UserService.class).selectUserCodeExist(userCode);
			if(null != user){
				resultMap.put("userCode","exist");
			}else{
...
//		outPrintWriter.close();
	...
	private void query(HttpServletRequest request, HttpServletResponse response)
    	...
    	//总数量（表）	
    	int totalCount	=  context.getBean("userServiceImpl", UserService.class).getUserCount(queryUserName,queryUserRole);
    	//总页数
		...
		userList =  context.getBean("userServiceImpl", UserService.class).getUserList(queryUserName,queryUserRole,currentPageNo, pageSize);
	...
	private void add(HttpServletRequest request, HttpServletResponse response)
		...
		if( context.getBean("userServiceImpl", UserService.class).add(user)){
			response.sendRedirect(request.getContextPath()+"/jsp/user.do?method=query");
		}else{
